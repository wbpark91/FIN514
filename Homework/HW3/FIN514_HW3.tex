...
\newcommand{\hmwkAuthorName}{Wanbae Park, Yongduk Kim, Inhyuk Lee} % Your name
...
\begin{homeworkProblem}
	...
	\end{figure}
	%% Exercise boundary: time-value of option part.
	Theoretically, the exercise boundary of a put option should be a smooth increasing function. It is because early exercise has a trade-off with time value of options. In other words, to exercise option early means intrinsic value is larger than time value of the option. Therefore, since time value of option decreases smoothly, exercise boundary should increase smoothly. However, as shown in figure, the shape of boundary is sawtoothing, which does not coincide with theory. Since the optimal exercise boundary is distorted under CRR method, exercise-or-not decision at each node will be distorted consequently, and finally it causes distortion of option prices.	\\
...
%----------------------------------------------------------------------------------------
\begin{homeworkProblem}
	Using the same method of Problem 3, we plotted errors of CRR model against steps. Figure \ref{fig:prob4-error} represents the behavior of errors of this model. $\Lambda$ is calculated at $t = 0.16$, which is when barrier can be applied. The red and black dot represents errors and $\Lambda$, and the horizontal black line is $\Lambda = 0.5$ line.
	%% Error of Barrier Option
	\begin{figure}[ht]
		\centering
		\includegraphics[scale = 0.5]{Q4/error.png}
		\caption{Error of CRR method}
		\label{fig:prob4-error}
	\end{figure}
	From this figure, the behavior of errors is similar to the error of continuous barrier option from Problem 3. It is periodic, and has jumps. It is observed that jumps occur at $\Lambda = 0$, which means as steps increases, barrier gets closer to some value of node, and if barrier gets slightly different to the value, jump occurs. The interesting thing is it appears that error is close to zero when $\Lambda$ is close to 0.5. $\Lambda = 0.5$ means that barrier is located between values of each node. Unlike the continuous barrier option where the error was minimized when $\Lambda$ was equal to zero, discrete barrier option has minimum error when $\Lambda$ is equal to 0.5. %%WHY????
\end{homeworkProblem}
\end{document}